...
This file tracks Phoenix-specific follow-up work now that TeleOp uses the fused Pinpoint + AprilTag localizer and the framework owns the fixed-tag policy.
...
2. Field-test and retune `RobotConfig.Localization.pinpointAprilTagFusion`:
   confirm the correction gains feel stable during normal TeleOp driving and after brief vision dropouts.
...
4. Keep scoring-tag selection intentionally limited to the fixed goal tags used for localization fallback.
