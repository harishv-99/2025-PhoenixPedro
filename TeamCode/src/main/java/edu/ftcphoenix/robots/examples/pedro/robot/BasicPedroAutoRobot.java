...
import edu.ftcphoenix.robots.examples.pedro.autonomous.BasicPedroAutoPaths;
import edu.ftcphoenix.robots.examples.pedro.autonomous.BasicPedroAutoRoutine;
...
public final class BasicPedroAutoRobot {
    ...
    public BasicPedroAutoRobot(RobotProgram program,
        ...
        registerPresenters(requiredProgram);
    }
    ...
    /** Return the latest retained route status for the additive presenter. */
    ...
    private void registerPresenters(RobotProgram program) {
        program.presenter((clock, frame) -> presentPlacementAndWarning(frame));
        program.presenter((clock, frame) -> presentRouteAndRootStatus(frame));
    }

    private void presentPlacementAndWarning(Telemetry frame) {
    ...
    private void presentRouteAndRootStatus(Telemetry frame) {
        frame.addData("example.routeStatus", latestRouteStatus());
        frame.addData("example.rootComplete", isRootComplete());
        frame.addData("example.rootOutcome", rootOutcome());
    }
