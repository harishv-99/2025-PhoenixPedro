...
Phoenix optimizes for **student simplicity without hiding truth**. Code should make the safe,
...
Clear ownership is the practical form of SOLID used by Phoenix: cohesive objects, narrow
...
- Framework Tasks reject a direct `update(clock)` before their first `start(clock)`. A runner may
  ...
  queue operation.
- Composition states lifetime explicitly. `parallelAll(...)` waits for every child;
...
for that sink. Route and guidance integrations select behavior through Phoenix seams without
...
Phoenix rejects invalid configuration at the first boundary that owns it. Runtime Plants expose
...
- Put frames, units, and signs in names when ambiguity is possible. Phoenix uses robot `+X` forward,
