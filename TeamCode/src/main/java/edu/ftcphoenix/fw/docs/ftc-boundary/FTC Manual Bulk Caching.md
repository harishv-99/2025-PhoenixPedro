...
deduplicated Phoenix-cycle invalidation attempts, and terminal cleanup inside the managed
`FtcRobotOpMode` lifecycle.

This owner is not installed by default, and Phoenix robot code does not currently adopt it. An
...
```java
import edu.ftcphoenix.fw.ftc.FtcBulkCaching;
...
- `AUTO` invalidates after a repeated identical SDK-tagged command, not at a Phoenix cycle boundary.
...
Consequently, the Phoenix owner promises only its own ordered invalidation attempts. It does not
...
This module-level cache is also separate from Phoenix
