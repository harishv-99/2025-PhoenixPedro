...
Sushi provides three small helpers for the most common cases:
...
Sushi provides a tiny helper for the "cap" part: `RequestCounter`.
...
Sushi provides a small helper: `Cooldown`.
...
Sushi provides `EnumStateMachine<S extends Enum<S>>`:
