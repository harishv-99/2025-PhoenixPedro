...
[`Robot roles`](<../getting-started/learn-sushi/Robot Roles.md>); this page expands that ownership
...
Sushi uses a few simple ownership rules.
...
6. **Subsystems own mechanism target resolvers and Plant update order.**
...
Sushi intentionally has no generic `Lane` interface, registry, or `updateAll()`. Vision,
...
- final target-resolver composition and Plant update order
...
A **subsystem** is a robot-owned mechanism owner and usually the single owner of a mechanism's target resolvers and Plant update order.
...
> Who owns this mechanism's target resolvers and Plant update order?
...
It usually does not own a mechanism's target resolvers or Plant update order, and it is usually not primarily about buttons.
...
Sushi also uses some lower-level runtime terms:
...
Sushi capability. The protected core never imports back out to an edge, and the robot
...
### Does it own one mechanism's target resolvers and Plant update order?
...
`GamepadDevice` and `Gamepads` are FTC adapters in `edu.ftcsushi.fw.ftc.input`; they are the small
edge that accepts SDK `Gamepad` objects and publishes Sushi sources. Robot code keeps the simple
...
derive mutually exclusive activation sources from one robot-owned mode. Sushi does not choose a
