...
Configuration containing the intended devices; only configured devices appear in the picker.

## Start here

1. Read [Hardware-free Reference Scenarios](<../examples/Hardware-free Reference Scenarios.md>).
   Follow request → heartbeat → recorded command, then name the unproven physical fact.
   Optionally run the check after setup.
...
| How do I estimate between accepted measurements? | [Calibration tables](<Interpolate Calibration Measurements.md>) | the calculation and its evidence limits are clear |
| How do I keep a completed result on my laptop? | [Download experiment results](<Download and Inspect Experiment Results.md>) | the saved file matches the lab card |
