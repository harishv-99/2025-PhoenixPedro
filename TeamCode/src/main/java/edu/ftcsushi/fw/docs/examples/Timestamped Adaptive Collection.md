...
| [`AdaptiveCollectionPaths`](<../../../robots/examples/pedro/adaptive/AdaptiveCollectionPaths.java>) | Sushi-to-Pedro target conversion, live Pedro start-pose snapshots, selected/fallback/return geometry, and two native semantic callbacks. | Camera interpretation, route execution policy, inventory, or follower lifecycle. |
