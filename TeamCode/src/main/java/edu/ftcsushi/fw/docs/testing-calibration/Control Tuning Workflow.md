...
- a list that Sushi automatically iterates;
...
4. On the first A, or any A after B/timeout, Sushi requests zero and waits non-blockingly for
   finite feedback plus `plant.atTarget(0.0)`.
5. If controller gains changed, Sushi applies the complete candidate. A software controller is
   reset and reseeded from the current measurement; an FTC controller is written and read back.
6. Sushi commits the captured target through the Plant's one command target and starts a numbered
...
SDK/firmware PIDF coefficient in native controller units; Sushi does not relabel it as physical
...
receive one target and intentionally share one PIDF candidate. Sushi supports that case without
...
3. A `needsReference(...)` Plant requires the four-argument factory. The first A creates and starts
   ...
   alive. Success must actually establish the coordinate reference; Sushi then stages current
...
current OpMode lifetime, Sushi keeps every completed segment's full candidate, controller
...
do not expose those internal terms, so Sushi marks them unavailable rather than inventing zeros.
...
tuner INIT Sushi replaces it with exactly the active controller and experiment fields, then
...
Because Configurables publishes entry values individually, Sushi waits for a short unchanged
