...
final class FtcWebcamAprilTagSupport {
    ...
    private static final Mat3 FTC_ROBOT_FROM_SUSHI_ROBOT = new Mat3(
            0, -1, 0,
            1, 0, 0,
            0, 0, 1
    );
    private static final Mat3 SUSHI_CAMERA_FROM_FTC_OPTICAL_CAMERA = new Mat3(
            0, 0, 1,
            -1, 0, 0,
            0, -1, 0
    );
    ...
    /** Converts Sushi mount semantics into the FTC SDK's robot/optical-camera convention. */
    static SdkCameraPose toSdkCameraPose(CameraMountConfig mount) {
        ...
        Vec3 ftcTranslation = FTC_ROBOT_FROM_SUSHI_ROBOT
                .mul(robotToCameraPose.translation());
        Mat3 sdkRotation = FTC_ROBOT_FROM_SUSHI_ROBOT
                .mul(robotToCameraPose.rotation())
                .mul(SUSHI_CAMERA_FROM_FTC_OPTICAL_CAMERA);
        ...
        private AprilTagDetections readDetections(
            ...
            for (AprilTagDetection detection : detections) {
                ...
                if (cameraToTagPose == null && detection.ftcPose != null) {
                    ...
                    cameraToTagPose = FtcFrames.toSushiFromFtcDetectionFrame(ftcCameraToTag);
                }
    ...
    private static Pose3d cameraToTagFromRawPose(AprilTagPoseRaw rawPose) {
        ...
        // FTC raw camera (+X right, +Y down, +Z forward) -> Sushi (+X forward, +Y left, +Z up).
        ...
        Mat3 cameraRotation =
                FtcFrames.sushiFromAprilTagRawCameraFrame().mul(rawRotation);
