 ...
 * <b>Sushi framing</b>.
 *
 * <h2>Principle: Sushi framing for Sushi-defined frames</h2>
 * <p>
 * All core Sushi framework types operate in Sushi's standard right-handed axis convention
 * for <b>Sushi-defined frames</b> (robot, camera, field, mechanisms, etc.):
 * </p>
 * <ul>
 *   <li><b>+X</b> forward</li>
 *   <li><b>+Y</b> left</li>
 ...
 * Sushi keeps that tag frame consistent with the FTC game database metadata so that
 * {@code fieldToTagPose} (from the layout) and {@code cameraToTagPose} (from the detector)
 * compose cleanly.
 * </p>
 *
 * <h2>Sushi pose naming convention</h2>
 * <p>
 * Whenever a variable or accessor represents a pose/transform, Sushi code uses:
 * </p>
 * <pre>
 ...
 * <p>
 * For a component derived from a pose's translation, Sushi uses:
 * </p>
 * <pre>
 ...
 * </ul>
 *
 * <h2>Sushi camera frame (for cameraToTagPose)</h2>
 * <p>
 * Sushi defines the camera frame using the same axis convention as all Sushi frames:
 * </p>
 * <ul>
...
public final class AprilTagObservation {
    ...
    /**
     * Tag pose expressed in the Sushi camera frame: {@code cameraToTagPose}.
     *
    ...
    /**
     * Optional robot pose expressed in the Sushi field frame: {@code fieldToRobotPose}.
     *
     * <p>If present, this is a field-centric pose measurement source (6DOF). For example, an FTC
     * adapter may populate this from SDK robotPose after converting into Sushi framing.</p>
     */
    ...
    /**
     * Create an observation representing a detected tag, expressed in Sushi framing.
     *
     ...
     * @param cameraToTagPose tag pose in Sushi camera frame (non-null)
     ...
     * @param id               AprilTag ID
     * @param cameraToTagPose  tag pose in Sushi camera frame (non-null)
     * @param fieldToRobotPose robot pose in Sushi field frame (non-null)
     * @return geometry-only target observation awaiting a frame timestamp
    ...
    // ---------------------------------------------------------------------------------------------
    // Components derived from cameraToTagPose (Sushi camera frame)
    // ---------------------------------------------------------------------------------------------

    /**
     * Tag forward component in the Sushi camera frame (+X forward), in inches.
     */
    ...
    /**
     * Tag left component in the Sushi camera frame (+Y left), in inches.
     */
    ...
    /**
     * Tag up component in the Sushi camera frame (+Z up), in inches.
     */
     ...
     *
     * <p>Derived from {@link #cameraToTagPose} using Sushi sign convention: positive = left.</p>
     */
