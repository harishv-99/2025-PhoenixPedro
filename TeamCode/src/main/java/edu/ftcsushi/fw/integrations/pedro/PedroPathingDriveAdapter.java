...
/**
 * Framework adapter that bridges Pedro Pathing into Sushi's small route/drive seams.
 *
 * <p>This adapter intentionally plays three closely related roles for one Pedro Follower:</p>
 * <ul>
 *   <li>{@link RouteFollower}&lt;{@link PathChain}&gt; so Sushi tasks can sequence Pedro routes.</li>
 *   <li>{@link DriveCommandSink} so Sushi guidance tasks (such as aim tasks) can temporarily
 ...
 *
 * <p>An ordinary managed Auto transfers this adapter into one application-owned service:</p>
 * <pre>{@code
 ...
 * }</pre>
 * <p>The robot-owned service applies the start pose at START, updates the runtime predictor before
 * this adapter, and owns the final physical zero. Route Tasks may share the hook because
 * same-cycle calls are deduplicated. The maintained Basic Pedro example contains the complete
 * service shape.</p>
...
public final class PedroPathingDriveAdapter implements RouteFollower<PathChain>, DriveCommandSink {
    ...
    /**
     * Creates a Sushi adapter around one Pedro {@link Follower} instance.
     *
     ...
     * runtimes that share Sushi localization use this package-private path so the passive Pedro
    ...
    /**
     * Advances the wrapped Follower at most once in the supplied Sushi loop cycle.
     *
     ...
     *
     * @param clock shared Sushi clock for the current OpMode cycle; must not be null
     */
    @Override
