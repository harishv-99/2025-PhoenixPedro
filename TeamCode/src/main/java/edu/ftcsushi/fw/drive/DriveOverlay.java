 ...
 *
 * <p>Overlays are the backbone of “driver assist” in Sushi. They are used for behaviors like:</p>
 * <ul>
 ...
 * <p>An overlay is typically applied using {@link DriveSource#overlayWhen(edu.ftcsushi.fw.core.source.BooleanSource, DriveOverlay, DriveOverlayMask)}.</p>
 * <p>The base {@link DriveSource} continues to run each loop. When enabled, the overlay produces a
 ...
 *
 * <p>One overlay instance has exactly one activation owner: use it in one
 * {@link DriveSource#overlayWhen} wrapper or one {@link DriveOverlayStack} layer. When the same
 * behavior is needed in two independently activated compositions, construct two overlay instances.
 * Sharing one instance would make its enable/disable transitions ambiguous.</p>
 *
 * <h2>Per-cycle state</h2>
 * <p>An overlay that owns controller, blend, filter, or other behavioral state must be idempotent
 * by {@link LoopClock#cycle()}. Repeated {@link #get(LoopClock)} calls in one cycle must return an
 * observationally identical result without advancing that state again. Composition owners cache
 * their completed drive result, but an overlay must still protect itself because an evaluation may
 * be retried after a downstream failure or the overlay may have another direct consumer.</p>
 */
public interface DriveOverlay {
     ...
     *
     * <p>Stateful implementations follow the per-cycle contract in this interface's class
     * documentation.</p>
     */
