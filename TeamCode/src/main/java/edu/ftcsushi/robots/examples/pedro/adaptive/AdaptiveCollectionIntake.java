...
/**
 * Cancellation-safe intake capability owned by the adaptive collection example.
 *
 ...
 * rewriting that graph. A real robot normally replaces this class with its existing mechanism
 * capability. Ordinary FTC construction
 ...
 */
public final class AdaptiveCollectionIntake implements RobotProgram.Output {
     ...
     */
    public AdaptiveCollectionIntake(HardwareMap hardwareMap, Config config) {
        HardwareMap requiredHardwareMap = Objects.requireNonNull(hardwareMap, "hardwareMap");
        Config source = Objects.requireNonNull(config, "AdaptiveCollectionIntake.Config is required");
     ...
     */
    AdaptiveCollectionIntake(Plant intakePlant) {
    ...
    private static Plant requireCommandPlant(Plant plant) {
        if (!plant.hasCommandTarget()) {
            throw new IllegalArgumentException(
                    "AdaptiveCollectionIntake requires intakePlant to have a command target"
            );
        }
    ...
    private static void requireCollectPower(double collectPower) {
        if (!Double.isFinite(collectPower)
                ...
                || collectPower > 1.0) {
            throw new IllegalArgumentException(
                    "AdaptiveCollectionIntake.Config.collectPower must be finite, nonzero, and in "
    ...
    private static void requireHardwareName(String value) {
        if (value == null || value.trim().isEmpty()) {
            throw new IllegalArgumentException(
                    "AdaptiveCollectionIntake.Config.motorName must be a non-blank FTC hardware "
                            + "name, got "
    ...
    private static void requireDirection(Direction direction) {
        if (direction == null) {
            throw new IllegalArgumentException(
                    "AdaptiveCollectionIntake.Config.direction is required, got null"
            );
        }
    }
}
