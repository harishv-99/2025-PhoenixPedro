...
public final class PedroFieldTransformTest {
    ...
    @Test
    public void publicApiUsesOnlyCurrentSushiFieldNames() {
        Set<String> publicMethods = new HashSet<String>();
        for (Method method : PedroFieldTransform.class.getDeclaredMethods()) {
            if (Modifier.isPublic(method.getModifiers())) {
                publicMethods.add(method.getName());
            }
        }

        assertTrue(publicMethods.contains("sushiFieldToPedroPose"));
        assertTrue(publicMethods.contains("pedroToSushiFieldPose"));
        assertTrue(publicMethods.contains("sushiFieldVelocityToPedro"));
        for (String method : publicMethods) {
            assertFalse("Former Phoenix field alias remains public: " + method,
    ...
    public void decodeCenterAxesAndHeadingsMatchExplicitContract() {
        assertPedroPose(72.0, 72.0, Math.PI / 2.0,
                transform.sushiFieldToPedroPose(new Pose2d(0.0, 0.0, 0.0)));
        assertPedroPose(72.0, 82.0, Math.PI / 2.0,
                transform.sushiFieldToPedroPose(new Pose2d(10.0, 0.0, 0.0)));
        assertPedroPose(62.0, 72.0, Math.PI,
                transform.sushiFieldToPedroPose(new Pose2d(0.0, 10.0, Math.PI / 2.0)));

        Pose2d sushi = transform.pedroToSushiFieldPose(new Pose(72.0, 72.0, 0.0));
        assertEquals(0.0, sushi.xInches, EPSILON);
        assertEquals(0.0, sushi.yInches, EPSILON);
        assertEquals(-Math.PI / 2.0, sushi.headingRad, EPSILON);
    }
    ...
    public void poseTransformIsAnInverseAcrossWrapBoundaries() {
        ...
        for (Pose2d original : cases) {
            Pose2d roundTrip = transform.pedroToSushiFieldPose(
                    transform.sushiFieldToPedroPose(original)
            );
    ...
    public void velocityRotatesWithoutFieldOriginTranslationAndRoundTrips() {
        Pose pedroVelocity = transform.sushiFieldVelocityToPedro(3.0, 4.0, 0.5);

        assertPedroPose(-4.0, 3.0, 0.5, pedroVelocity);
        PedroFieldTransform.SushiFieldVelocity roundTrip =
                transform.pedroFieldVelocityToSushi(pedroVelocity);
    ...
    public void inverseRejectsAmbiguousVendorCoordinateTags() {
        try {
            transform.pedroToSushiFieldPose(
    ...
    public void transformRejectsNonFiniteComponents() {
        try {
            transform.sushiFieldToPedroPose(new Pose2d(Double.NaN, 0.0, 0.0));
            fail("Expected non-finite Sushi pose rejection");
        } catch (IllegalArgumentException expected) {
            assertTrue(expected.getMessage().contains("xInches"));
        }

        try {
            transform.sushiFieldVelocityToPedro(0.0, Double.POSITIVE_INFINITY, 0.0);
