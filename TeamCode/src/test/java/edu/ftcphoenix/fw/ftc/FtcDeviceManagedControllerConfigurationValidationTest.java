...
public final class FtcDeviceManagedControllerConfigurationValidationTest {
    ...
    @Test
    public void velocityTuningIsOneAnswerAndPositionRetainsItsMultiSettingExit()
            throws Exception {
        ...
        for (Method method : FtcActuators.MotorDeviceManagedVelocityStep.class
                .getDeclaredMethods()) {
            assertFalse("velocity tuning must not retain an administrative exit",
                    "doneDeviceManaged".equals(method.getName()));
        }
    ...
    public void rejectedTupleDoesNotCommitAndValidRetryForwardsExactValuesAndSignedZeros() {
        ...
        Plants.PositionPeriodicityStep<Plants.FeedbackPositionBoundsStep> periodicity = position
                .outerPositionP(-0.0)
                .innerVelocityPidf(
    ...
    @Test
    public void stagedMaxPowerRejectsOutsideNormalizedDomainAndAllowsExactRetry() {
        TestHardwareMap hardwareMap = mapWithMotor("arm");
        FtcActuators.MotorDeviceManagedPositionStep tuning =
                positionTuning(hardwareMap, "arm");
        for (double invalid : invalidMaxPowers()) {
            assertExactIllegalArgument(maxPowerDiagnostic("FtcActuators.maxPower(...)", invalid),
                    () -> tuning.maxPower(invalid));
            hardwareMap.assertNoEffects();
        }

        PositionPlant plant = finishPosition(
                tuning.maxPower(-0.0).doneDeviceManaged(), 12.0);
        ...
        for (double valid : new double[]{+0.0, 0.25, 1.0}) {
            ...
            PositionPlant accepted = finishPosition(
                    positionTuning(acceptedMap, "arm")
                            .maxPower(valid)
                            .doneDeviceManaged(),
                    1.0);
    ...
    @Test
    public void explicitMotorPositionOverloadsValidateMaxPowerBeforeLookupOrDirection() {
        TestHardwareMap namedMap = mapWithMotor("arm");
        for (double invalid : invalidMaxPowers()) {
            assertExactIllegalArgument(
                    maxPowerDiagnostic("FtcHardware.motorPosition(...)", invalid),
                    () -> FtcHardware.motorPosition(
                            namedMap, "arm", Direction.FORWARD, invalid));
            namedMap.assertNoEffects();
        }

        PositionOutput named = FtcHardware.motorPosition(
                namedMap, "arm", Direction.FORWARD, -0.0);
        assertEquals(1, namedMap.lookupCalls);
        assertEquals(1, namedMap.motor("arm").directionWrites);
        named.setPosition(8.0);
        ...
        for (double invalid : invalidMaxPowers()) {
            assertExactIllegalArgument(
                    maxPowerDiagnostic("FtcHardware.motorPosition(...)", invalid),
                    () -> FtcHardware.motorPosition(
                            directProbe.motor, Direction.REVERSE, invalid));
            directProbe.assertNoEffects();
        }

        PositionOutput direct = FtcHardware.motorPosition(
                directProbe.motor, Direction.REVERSE, 0.5);
        assertEquals(1, directProbe.directionWrites);
        direct.setPosition(9.0);
    ...
    public void deviceToleranceEnforcesUnsigned16BitDomainAndPreservesRetryAndEndpoints() {
        ...
        finishPosition(retry.devicePositionToleranceTicks(0).doneDeviceManaged(), 0.0);
    ...
    public void positionTuningRequiresNonemptyClosedSectionAndClosedAliasesCannotMutate() {
        ...
        Throwable empty = assertThrows(IllegalStateException.class, retained::doneDeviceManaged);
        assertContainsIgnoreCase(empty, "at least", "override");
        hardwareMap.assertNoEffects();

        Plants.PositionPeriodicityStep<Plants.FeedbackPositionBoundsStep> closed = retained
                .maxPower(0.4)
                .doneDeviceManaged();
        Throwable repeatedClose =
                assertThrows(IllegalStateException.class, retained::doneDeviceManaged);
        assertContainsIgnoreCase(repeatedClose, "closed");
        Throwable mutationAfterClose = assertThrows(
                IllegalStateException.class, () -> retained.maxPower(Double.NaN));
        assertContainsIgnoreCase(mutationAfterClose, "closed");
        hardwareMap.assertNoEffects();

        PositionPlant plant = finishPosition(closed, 4.0);
        plant.update(new ManualLoopClock().clock());
    ...
    public void distinctPositionSettingsAreOrderIndependentAndDuplicatesPreserveFirstAnswers() {
        ...
        assertDuplicate(() -> retained.maxPower(Double.NaN));
        assertDuplicate(() -> retained.outerPositionP(Double.NaN));
        hardwareMap.assertNoEffects();

        PositionPlant plant = finishPosition(retained.doneDeviceManaged(), 2.0);
        plant.update(new ManualLoopClock().clock());
        MotorProbe motor = hardwareMap.motor("arm");
    ...
    public void acceptedVelocityAnswerClosesRetainedTuningAliasWithoutReplacingTuple() {
        ...
        Plants.VelocityBoundsStep bounds = retained.velocityPidf(
    ...
    @Test
    public void defaultsResultsCannotBeCastBackIntoTuningAndRemainUsableAfterRejection() {
        TestHardwareMap velocityMap = mapWithMotor("flywheel");
        Plants.VelocityBoundsStep velocityDefaults = FtcActuators.plant(velocityMap)
                .motor("flywheel", Direction.FORWARD)
                .velocity()
                .deviceManagedWithDefaults();
        FtcActuators.MotorDeviceManagedVelocityStep velocityAlias =
                (FtcActuators.MotorDeviceManagedVelocityStep) velocityDefaults;
        Throwable velocityFailure = assertThrows(IllegalStateException.class,
                () -> velocityAlias.velocityPidf(1.0, 2.0, 3.0, 4.0));
        assertContainsIgnoreCase(velocityFailure, "deviceManaged");
        velocityMap.assertNoEffects();
        finishVelocity(velocityDefaults);
        velocityMap.motor("flywheel").assertNoOptionalControllerWrites();

        TestHardwareMap positionMap = mapWithMotor("arm");
        Plants.PositionPeriodicityStep<Plants.FeedbackPositionBoundsStep> positionDefaults =
                FtcActuators.plant(positionMap)
                        .motor("arm", Direction.FORWARD)
                        .position()
                        .deviceManagedWithDefaults();
        FtcActuators.MotorDeviceManagedPositionStep positionAlias =
                (FtcActuators.MotorDeviceManagedPositionStep) positionDefaults;
        Throwable positionFailure = assertThrows(IllegalStateException.class,
                () -> positionAlias.maxPower(0.25));
        assertContainsIgnoreCase(positionFailure, "deviceManaged");
        positionMap.assertNoEffects();
    ...
    public void castBypassesStillFailFinalPreflightBeforeHardwareResolution() {
        ...
        assertContainsIgnoreCase(positionFailure, "doneDeviceManaged");
        positionMap.assertNoEffects();
    }
    ...
    private static Plant finishVelocity(Plants.VelocityBoundsStep bounds) {
        return bounds
                .unbounded()
                .nativeUnits()
    ...
    private static double[] invalidMaxPowers() {
        return new double[]{
                Double.NaN,
                Double.NEGATIVE_INFINITY,
                Double.POSITIVE_INFINITY,
    ...
    private static String maxPowerDiagnostic(String operation, double value) {
        return operation
